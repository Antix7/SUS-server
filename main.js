...
function build_thead_sprzet_mod(ob) {
...
function build_table_sprzet_mod(ob) {
...
async function main() {
  ...
  // page for modifying the main database
  app.get('/sprzet_panel/modyfikuj', async function (request, response) {
  ...
  app.get('/sprzet_panel/modyfikuj/filters', async function (request, response) {
    if (!request.session.loggedin) {
      return;
    }
  ...
  app.post('/sprzet_panel/modyfikuj/filters/stany', async function(request, response) {
    if (!request.session.loggedin) {
      return;
    }
  ...
  app.post('/sprzet_panel/modyfikuj/auth', async function (request, response){
    if (!request.session.loggedin) {
      response.sendFile(__dirname + '/login/oszust.html');
      return;
    }
    ...
    let [rows, columns] = await con.execute(query);
    response.send(build_thead_sprzet_mod(rows)+build_table_sprzet_mod(rows));

  });
  // TODO not loading the whole table at once

  app.post('/sprzet_panel/modyfikuj/usun', function (request, response) {
    let conditions = [];
